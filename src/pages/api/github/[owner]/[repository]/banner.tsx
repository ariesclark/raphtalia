	...
	await createPageImage({
		...
		<body className="w-screen h-screen overflow-hidden bg-black text-neutral-100">
			<div className="flex flex-col w-screen h-screen overflow-hidden">
				<div className="absolute flex flex-col justify-between w-full h-screen p-4 overflow-hidden filter gap-y-2">
					{commits.filter(({ commit }: any) => !commit.message.toLowerCase().includes("merge branch"))
						.slice(0, 8).map(({ sha, commit, author }: any) => (
